...
def plot_graphs(G, G_, label=None, ticks_labels=None):
    if type(G)!=np.ndarray:
        G = nx.to_numpy_matrix(G)
    else: 
        G = G
    if type(G_)!=np.ndarray:
        G_ = nx.to_numpy_matrix(G_)
    else: 
        G_ = G_
    _max, _min = np.amax([G,G_]),np.amin([G,G_])
    fig = plt.figure(figsize=(12,12))
    fig.suptitle(label)
    ax1 = fig.add_subplot(2, 2, 1)
    ax2 = fig.add_subplot(2, 2, 2)
    mat1 = ax1.imshow(G,cmap='viridis', vmax=_max, vmin=_min)
    mat2 = ax2.imshow(G_,cmap='viridis', vmax=_max, vmin=_min)
    ax1.title.set_text('Original graph')
    ax2.title.set_text('Model')
    
    cbar_ax = fig.add_axes([0.95,0.53, 0.05, 0.35])
    fig.colorbar(mat1, cax=cbar_ax)
    
    ax3 = fig.add_subplot(2, 2, 3)
    ax3.title.set_text('Edge weight error')
    mat3 = ax3.imshow(np.abs(G_-G),cmap='Greys', vmin=0)
    cbar_ax2 = fig.add_axes([0.95,0.12, 0.05, 0.17])
    fig.colorbar(mat3, cax=cbar_ax2)
    ax4 = fig.add_subplot(2, 2, 4)
    ax4.title.set_text('Edge error')
    mat4_data = np.matrix(np.zeros(G.shape))
    colors=['white']
    labels = ['Small error']
    if mat4_data[np.logical_and(G_==0, G!=0)].size>0:
        mat4_data[np.logical_and(G_==0, G!=0)]=1
        colors.append('#983dff')
        labels.append('Missed')
    if mat4_data[np.logical_and(G_!=0, G==0)].size>0:
        mat4_data[np.logical_and(G_!=0, G==0)]=2
        colors.append('#3dfff9')
        labels.append('Invented')
    if mat4_data[np.multiply(G_,G)<0].size>0:
        mat4_data[np.multiply(G_,G)<0]=3
        colors.append('#ff9b3d')
        labels.append('Inversed')
    cmap_all = mpl.colors.LinearSegmentedColormap.from_list('Custom cmap', colors)
    if len(colors)>1:
        cmap = discrete_cmap(len(colors),cmap_all)
        mat4 = ax4.imshow(mat4_data,cmap=cmap)
        cbar_ax3 = fig.add_axes([0.95,0.3, 0.05, 0.17])
        cbar4=fig.colorbar(mat4, cax=cbar_ax3, ticks=np.arange(0, len(labels), 1))
        cbar4.ax.set_yticklabels(labels)
    if not ticks_labels:
        ticks_labels = np.arange(1, np.shape(G)[0]+1, 1)
    for ax in [ax1, ax2, ax3, ax4]:
         # Major ticks
        ax.set_xticks(np.arange(0, np.shape(G)[0], 1));
        ax.set_yticks(np.arange(0, np.shape(G)[0], 1));
        ...
        # Minor ticks
        ax.set_xticks(np.arange(-.5, np.shape(G)[0], 1), minor=True);
        ax.set_yticks(np.arange(-.5, np.shape(G)[0], 1), minor=True);
        #ax.grid(which='minor', color='w', linestyle='-', linewidth=1)
   #plt.show()


def compare_multiple_Gs(Gs, G_positions, G_original=None, label=None, labels_Gs=None, figsize=(12,4)):
    ...
    if G_original:
        ...
        axG.title.set_text('Original')
        plot_example(G_original, G_positions, ax=axG)
    for index, G in enumerate(Gs):
        index_cell = 1+index
        if n_rows == 2:
            index_cell = 1+len(Gs)+index
        axG = fig.add_subplot(n_rows, len(Gs), index_cell)
        if labels_Gs:
            axG.title.set_text(labels_Gs[index])
        plot_example(G, G_positions, axG)


def plot_example(G, G_positions, ax=None):
    labels = nx.get_edge_attributes(G,'weight')
    nx.draw_networkx_nodes(G, G_positions, ax=ax)
    nx.draw_networkx_labels(G, G_positions, font_color='white', ax=ax)
    nx.draw_networkx_edges(G, G_positions, ax=ax)
    nx.draw_networkx_edge_labels(G,G_positions, rotate=False, edge_labels=labels, ax=ax)
