...
default_args = easydict.EasyDict({
    # -----------data parameters ------
        ...
        "graph_sem_type":'linear-gauss', # {linear-gauss,linear-exp,linear-gumbel}
        "graph_linear_type":'linear', #linear -> linear SEM, nonlinear_1 -> x=Acos(x+1)+z, nonlinear_2 -> x=2sin(A(x+0.5))+A(x+0.5)+z')
        "edge_types":2,
        "x_dims":1,
        "z_dims":1,
        "optimizer":'Adam',
    ...
    # -----------training hyperparameters   
        ...
        "seed": 43,
        "epochs": 10,
        "batch_size": 10, # note: should be divisible by sample size, otherwise throw an error
...
class Model:
    def __init__(self, args_in):
        ...
        if self.args.graph_truth:
            self.args.data_variable_size = len(self.args.graph_truth.nodes())
    ...
    def train(self,epoch, best_val_loss, ground_truth_G, lambda_A, c_A, optimizer):
        t = time.time()
        nll_train = []
        kl_train = []
        mse_train = []
        ...
        # update optimizer
        self.optimizer, lr = self.update_optimizer(self.optimizer, self.args.lr, c_A)
        ...
        for batch_idx, (data, relations) in enumerate(self.train_loader):
            ...
            # compute h(A)
            h_A = self._h_A(origin_A, self.args.data_variable_size)
            ...
            myA.data = self.stau(myA.data, self.args.tau_A*lr)
...
# %%
def create_model_and_train(*args, **kargs):
    # seed=42, data_variable_size=10, graph_degree=3, 
    # data_sample_size=5000, k_max_iter=1e2, epochs=200
    t_total = time.time()
    ...
    c_A = model.args.c_A
    lambda_A = model.args.lambda_A
    h_A_new = torch.tensor(1.)
    h_tol = model.args.h_tol
    k_max_iter = int(model.args.k_max_iter)
    h_A_old = np.inf
    ...
    try:
        for step_k in range(k_max_iter):
            while c_A < 1e+20:
                for epoch in range(model.args.epochs):
                    ELBO_loss, NLL_loss, MSE_loss, graph, origin_A = model.train(epoch, best_ELBO_loss, model.ground_truth_G, lambda_A, c_A, model.optimizer)
                ...
                h_A_new = model._h_A(A_new, model.args.data_variable_size)
        ...
        '''
        if model.args.save_folder:
            print("Best Epoch: {:04d}".format(best_epoch), file=log)
            #log.flush()
        
        # test()
        print (best_ELBO_graph)
        print(nx.to_numpy_array(model.ground_truth_G))
        fdr, tpr, fpr, shd, nnz = count_accuracy(model.ground_truth_G, nx.DiGraph(best_ELBO_graph))
        print('Best ELBO Graph Accuracy: fdr', fdr, ' tpr ', tpr, ' fpr ', fpr, 'shd', shd, 'nnz', nnz)

        print(best_NLL_graph)
        print(nx.to_numpy_array(model.ground_truth_G))
        fdr, tpr, fpr, shd, nnz = count_accuracy(model.ground_truth_G, nx.DiGraph(best_NLL_graph))
        print('Best NLL Graph Accuracy: fdr', fdr, ' tpr ', tpr, ' fpr ', fpr, 'shd', shd, 'nnz', nnz)


        print (best_MSE_graph)
        print(nx.to_numpy_array(model.ground_truth_G))
        fdr, tpr, fpr, shd, nnz = count_accuracy(model.ground_truth_G, nx.DiGraph(best_MSE_graph))
        ...
        print('Best MSE Graph Accuracy: fdr', fdr, ' tpr ', tpr, ' fpr ', fpr, 'shd', shd, 'nnz', nnz)
        '''
    return model.ground_truth_G, best_ELBO_graph, count_accuracy(model.ground_truth_G, nx.DiGraph(best_ELBO_graph)), best_ELBO_loss


# %%
#ground_truth_G, best_ELBO_graph, acc = create_model_and_train(seed=42, data_variable_size=10, graph_degree=3, data_sample_size=5000)
